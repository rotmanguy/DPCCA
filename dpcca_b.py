...
if __name__ == '__main__':
    ...
    if cfg.resume:
        ...
        if os.path.isfile(cfg.resume):
            ...
            model_Z = Net(cfg.layer_sizes_Z, cfg.activ)
        ...
        Z_init = model_Z(Variable(from_numpy(batch[2]), requires_grad=False))
        ...
        for epoch in range(cfg.start_epoch,cfg.training_epochs):
            ...
            if epoch % cfg.display_val_step == 0:
                ...
                Z_val = model_Z(Variable(from_numpy(batch[2]), volatile=True))
                ...
                if is_best:
                    ...
                    save_checkpoint({'epoch': epoch + 1, 'state_dict_F': model_F.state_dict(),
                                     'state_dict_G': model_G.state_dict(), 'state_dict_Z': model_Z.state_dict(),
                                     'optimizer_F': optimizer_F.state_dict(), 'optimizer_G': optimizer_G.state_dict(),
                                     'batch_size_train': cfg.batch_size_train, 'epsilon': cfg.epsilon, 'lr': cfg.lr,
                                     'rho': cfg.rho, 'n_input_sizes': cfg.n_input_sizes,
                                     'layer_sizes_F': cfg.layer_sizes_F, 'layer_sizes_G': cfg.layer_sizes_G,
                                     'activ': cfg.activ},
                                    './outputs/dpcca_b_' + cfg.comment + cfg.dname_ +
                                    '_F_res_' + str(F_res) + '_G_res_' + str(G_res) + '_split_' + split + '_dict.pth.tar')
